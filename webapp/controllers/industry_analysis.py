...
@login_required
def industry_status(industry_gicscode_1="15",industry_gicscode_2="1510",industry_gicscode_3="151010",industry_gicscode_4="25102010",parameter=4): # 默认显示第四级分类
    parameter=parameter
    ...
    if parameter == 1:
        ...
        rs = session.query(func.sum(finance_basics_add.tot_oper_rev).label("tot_oper_rev"),
            ...
            cns_sub_industry.belong == cns_industry.industry_gicscode_3).filter(cns_industry.belong==cns_group_industry.industry_gicscode_2).filter(cns_group_industry.belong==cns_department_industry.industry_gicscode_1).filter(
            cns_department_industry.industry_gicscode_1 == industry_gicscode_1).group_by(finance_basics_add.the_year).all()
    if parameter == 2:
        ...
        rs = session.query(func.sum(finance_basics_add.tot_oper_rev).label("tot_oper_rev"),
                ...
                finance_basics_add.trade_code == cns_stock_industry.trade_code).filter(
                cns_stock_industry.industry_gicscode_4 == cns_sub_industry.industry_gicscode_4).filter(
                cns_sub_industry.belong == cns_industry.industry_gicscode_3).filter(cns_industry.belong==cns_group_industry.industry_gicscode_2).filter(
    ...
    if parameter == 3:
        ...
        rs = session.query(func.sum(finance_basics_add.tot_oper_rev).label("tot_oper_rev"),func.sum(finance_basics_add.net_profit_is).label("net_profit_is"),func.sum(finance_basics_add.wgsd_net_inc).label("wgsd_net_inc"),func.sum(finance_basics_add.tot_assets).label("tot_assets"),func.sum(finance_basics_add.tot_liab).label("tot_liab"),func.sum(finance_basics_add.wgsd_com_eq).label("wgsd_com_eq"),func.sum(finance_basics_add.operatecashflow_ttm2).label("operatecashflow_ttm2"),func.sum(finance_basics_add.investcashflow_ttm2).label("investcashflow_ttm2"),func.sum(finance_basics_add.financecashflow_ttm2).label("financecashflow_ttm2"),func.sum(finance_basics_add.cashflow_ttm2).label("cashflow_ttm2"),func.avg(finance_basics_add.net_profit_rate).label("net_profit_rate"),func.avg(finance_basics_add.tot_assets_turnover).label("tot_assets_turnover"),func.avg(finance_basics_add.equ_multi).label("equ_multi"),func.avg(finance_basics_add.roe_tot).label("roe_tot"),func.avg(finance_basics_add.roe_holder).label("roe_holder"),func.count(finance_basics_add.trade_code).label("company_num"),cns_industry.industry_gics_3.label("industry_gics_3")).filter(finance_basics_add.trade_code == cns_stock_industry.trade_code).filter(cns_stock_industry.industry_gicscode_4 == cns_sub_industry.industry_gicscode_4).filter(cns_sub_industry.belong == cns_industry.industry_gicscode_3).filter(cns_industry.industry_gicscode_3 == industry_gicscode_3).group_by(finance_basics_add.the_year).all()
    if parameter == 4:
        ...
        rs = session.query(func.sum(finance_basics_add.tot_oper_rev).label("tot_oper_rev"),func.sum(finance_basics_add.net_profit_is).label("net_profit_is"),func.sum(finance_basics_add.wgsd_net_inc).label("wgsd_net_inc"),func.sum(finance_basics_add.tot_assets).label("tot_assets"),func.sum(finance_basics_add.tot_liab).label("tot_liab"),func.sum(finance_basics_add.wgsd_com_eq).label("wgsd_com_eq"),func.sum(finance_basics_add.operatecashflow_ttm2).label("operatecashflow_ttm2"),func.sum(finance_basics_add.investcashflow_ttm2).label("investcashflow_ttm2"),func.sum(finance_basics_add.financecashflow_ttm2).label("financecashflow_ttm2"),func.sum(finance_basics_add.cashflow_ttm2).label("cashflow_ttm2"),func.avg(finance_basics_add.net_profit_rate).label("net_profit_rate"),func.avg(finance_basics_add.tot_assets_turnover).label("tot_assets_turnover"),func.avg(finance_basics_add.equ_multi).label("equ_multi"),func.avg(finance_basics_add.roe_tot).label("roe_tot"),func.avg(finance_basics_add.roe_holder).label("roe_holder"),func.count(finance_basics_add.trade_code).label("company_num"),cns_sub_industry.industry_gicscode_4,cns_sub_industry.industry_gics_4.label("industry_gics_4")).filter(finance_basics_add.trade_code == cns_stock_industry.trade_code).filter(cns_stock_industry.industry_gicscode_4 == cns_sub_industry.industry_gicscode_4).filter(cns_sub_industry.industry_gicscode_4 == industry_gicscode_4).group_by(finance_basics_add.the_year).all()
#计算rs的长度
    rs_list=range(len(rs))
    rs_list.reverse()
    return render_template("industry_analysis/industry_status.html",parameter=parameter,cns_filterform1=cns_filterform1,cns_filterform2=cns_filterform2,cns_filterform3=cns_filterform3,cns_filterform4=cns_filterform4,year_list=year_list,rs=rs,rs_list=rs_list)
...
# filter(cns_department_industry.industry_gicscode_1=='10') 筛选行业类别OK
    ...
    return render_template("industry_analysis/industry_classify.html",cns_filterform1=cns_filterform1,cns_filterform2=cns_filterform2,cns_filterform3=cns_filterform3,cns_filterform4=cns_filterform4,result=result,pagination=pagination,v_stock_industry=v_stock_industry, length=length)
...
def cns_filter():
    ...
    if cns_filterform1.validate_on_submit():
       ...
       # industry_gics_1 = request.form.get('industry_gics_1')
        ...
        result = pagination.items
        length = len(result)
        return render_template("industry_analysis/industry_classify_filter.html", result=result, pagination=pagination, length=length,cns_filterform1=cns_filterform1,cns_filterform2=cns_filterform2,cns_filterform3=cns_filterform3,cns_filterform4=cns_filterform4)
    if cns_filterform2.validate_on_submit():
        gics_code=request.form.get('gics_code')
        page = request.args.get('page', 1, type=int)
...
def annual_table(year='20151231'):
    ...
    page = request.args.get('page', 1, type=int)
    pagination = finance_basics_add.query.filter(finance_basics_add.the_year==year).order_by(finance_basics_add.trade_code).paginate(page,per_page=200,error_out=False)
    rs = pagination.items
    length = len(rs)
    return render_template("industry_analysis/annual_table.html",year=year,year_form=year_form,rs=rs,pagination=pagination,length=length)

    # 测试用
    # rs1 = finance_basics_add.query.filter(finance_basics_add.the_year=='20151231').join(cns_stock_industry).join(cns_sub_industry).add_columns(cns_sub_industry.industry_gics_4).join(cns_industry).add_columns(cns_industry.industry_gics_3).join(cns_group_industry).add_columns(cns_group_industry.industry_gics_2).join(cns_department_industry).add_columns(cns_department_industry.industry_gics_1).join(zhengjianhui_1).add_columns(zhengjianhui_1.industry_CSRC12).order_by(finance_basics_add.trade_code).all()

# 用时特别长的代码（合在一起写的）
# rs_finance = finance_basics_add.query.filter(finance_basics_add.the_year==year).join(cns_stock_industry).join(cns_sub_industry).add_columns(cns_sub_industry.industry_gics_4).join(cns_industry).add_columns(cns_industry.industry_gics_3).join(cns_group_industry).add_columns(cns_group_industry.industry_gics_2).join(cns_department_industry).add_columns(cns_department_industry.industry_gics_1).join(zhengjianhui_1).add_columns(zhengjianhui_1.industry_CSRC12).order_by(finance_basics_add.trade_code).all()
...
def annual_table_b(year='20151231',parameter=4): # 默认是2015年
    ...
    rs_all = session.query(func.sum(finance_basics_add.tot_oper_rev).label("all_tot_oper_rev"),
                       ...
                       func.sum(finance_basics_add.tot_assets - finance_basics_add.tot_liab).label("all_net_assets"),
    ...
    if parameter == 4:
        if year_form.validate_on_submit():
            ...
            year = year
        rs = session.query(func.sum(finance_basics_add.tot_oper_rev).label("tot_oper_rev"),func.sum(finance_basics_add.net_profit_is).label("net_profit_is"),func.sum(finance_basics_add.wgsd_net_inc).label("wgsd_net_inc"),func.sum(finance_basics_add.tot_assets).label("tot_assets"),func.sum(finance_basics_add.tot_liab).label("tot_liab"),func.sum(finance_basics_add.tot_assets-finance_basics_add.tot_liab).label("net_assets"),func.sum(finance_basics_add.wgsd_com_eq).label("wgsd_com_eq"),func.max(finance_basics_add.tot_oper_rev).label("mx_tot_oper_rev"),func.max(finance_basics_add.net_profit_is).label("mx_net_profit_is"),func.max(finance_basics_add.wgsd_net_inc).label("mx_wgsd_net_inc"),func.max(finance_basics_add.tot_assets).label("mx_tot_assets"),func.max(finance_basics_add.tot_liab).label("mx_tot_liab"),func.max(finance_basics_add.tot_assets - finance_basics_add.tot_liab).label("mx_net_assets"),func.max(finance_basics_add.wgsd_com_eq).label("wgsd_com_eq"),func.sum(finance_basics_add.operatecashflow_ttm2).label("operatecashflow_ttm2"),func.sum(finance_basics_add.investcashflow_ttm2).label("investcashflow_ttm2"),func.sum(finance_basics_add.financecashflow_ttm2).label("financecashflow_ttm2"),func.sum(finance_basics_add.cashflow_ttm2).label("cashflow_ttm2"),func.sum(finance_basics_add.free_cash_flow).label("free_cash_flow"),func.avg(finance_basics_add.net_profit_rate).label("net_profit_rate"),func.avg(finance_basics_add.tot_assets_turnover).label("tot_assets_turnover"),func.avg(finance_basics_add.equ_multi).label("equ_multi"),func.avg(finance_basics_add.roe_tot).label("roe_tot"),func.avg(finance_basics_add.roe_holder).label("roe_holder"),func.count(finance_basics_add.trade_code).label("company_num"),cns_sub_industry.industry_gicscode_4.label("industry_gicscode_4"),cns_sub_industry.industry_gics_4.label("industry_gics_4")).filter(finance_basics_add.trade_code == cns_stock_industry.trade_code).filter(cns_stock_industry.industry_gicscode_4 == cns_sub_industry.industry_gicscode_4).filter(finance_basics_add.the_year == year).group_by(cns_sub_industry.industry_gicscode_4).all()
        length = len(rs)
# 分组后，计算7个财务指标的组最大值
    list_tot_oper_rev = []
    ...
    return render_template("industry_analysis/annual_table_b.html",parameter=parameter,year=year,year_form=year_form,rs=rs,rs_all=rs_all,length=length,max_tot_oper_rev=max_tot_oper_rev,max_net_profit_is=max_net_profit_is,max_wgsd_net_inc=max_wgsd_net_inc,max_tot_assets=max_tot_assets,max_tot_liab=max_tot_liab,max_net_assets=max_net_assets,max_wgsd_com_eq=max_wgsd_com_eq)
...
# 做一个year_list，以万科A为标准
    year_list = []
    wanke_latest = session.query((finance_basics_add.the_year).label("the_year")).filter(finance_basics_add.sec_name=='万科A').order_by(finance_basics_add.the_year.desc()).first()
...
# 做一个结果集
    ...
    # rs = session.query(func.sum(finance_basics_add.tot_oper_rev).label("tot_oper_rev"),func.sum(finance_basics_add.net_profit_is).label("net_profit_is"),func.sum(finance_basics_add.wgsd_com_eq).label("wgsd_com_eq"),func.sum(finance_basics_add.tot_assets).label("tot_assets"),func.sum(finance_basics_add.tot_liab).label("tot_liab"),func.sum(finance_basics_add.wgsd_com_eq).label("wgsd_com_eq"),func.sum(finance_basics_add.operatecashflow_ttm2).label("operatecashflow_ttm2"),func.sum(finance_basics_add.investcashflow_ttm2).label("investcashflow_ttm2"),func.sum(finance_basics_add.financecashflow_ttm2).label("financecashflow_ttm2"),func.sum(finance_basics_add.cashflow_ttm2).label("cashflow_ttm2"),func.avg(finance_basics_add.net_profit_rate).label("net_profit_rate"),func.avg(finance_basics_add.tot_assets_turnover).label("tot_assets_turnover"),func.avg(finance_basics_add.equ_multi).label("equ_multi"),func.avg(finance_basics_add.roe_tot).label("roe_tot"),func.avg(finance_basics_add.roe_holder).label("roe_holder"),func.count(finance_basics_add.trade_code).label("company_num"),finance_basics_add.the_year.label("the_year")).group_by(finance_basics_add.the_year).all()


#  wanke_latest = session.query((finance_basics_add.the_year).label("the_year")).filter(finance_basics_add.sec_name=='万科A').order_by(finance_basics_add.the_year.desc()).first()
# year = wanke_latest.the_year[:4]

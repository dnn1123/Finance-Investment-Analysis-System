...
from .bpm import handle_form, handle_liveform, dict_to_sql, sql_to_dict, Strategy_Manager, Strategy, \
    create_position_records, get_stock_list, get_fiducial_value_data
...
def back_test():
    ...
    plotter_data = get_fiducial_value_data(string.atof(request.form.get('cash').encode('utf-8')), plotter_date,
                                           stock_list, string.atof(request.form.get('commission').encode('utf-8')))
    # plotter_data=[]
    res = {
...
def get_realtime_simulation_data():
    ...
    for each in data:
        result.append(
            [each[0].identifier, each[0].strategy_name, each.name_cn, sql_to_dict(each[0].parameter)['cash'], 'N/A',
             each[0].status, each[0].build_date.strftime("%Y年%m月%d日"), ''])
...
def get_subscribe_strategy_info():
    id = request.args.get('id')
    data = subscriber.query.filter_by(identifier=id).join(strategy).add_columns(strategy.name_cn, strategy.type,
                                                                                strategy.name_en).first()
    delta = datetime.datetime.now() - data[0].build_date
    result = {"strategy_name": data[0].strategy_name, "params": sql_to_dict(data[0].parameter),
              "status": data[0].status, "build_date": data[0].build_date.strftime("%Y-%m-%d"), "str_name": data.name_cn,
              "type": data.type, "str_name_en": data.name_en, "period": delta.days}
...
def get_models():
    ...
    for each in data:
        result.append(
            {"name_en": each.name_en, "name_cn": each.name_cn, "type": each.type, "description": each.description,
             "id": each.id})
    return jsonify(result)

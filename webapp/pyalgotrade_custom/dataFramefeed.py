# -*- coding: utf-8 -*-
#用以转换dataFrame到feed，相当于以pandas dataframe 为桥，不再以csv为桥。下一步增加方法直接从数据库中读
...
class RowParser(dataFrameBarfeed.RowParser):
    ...
    def handler(x):
        pass
    #row的结构 row[0]为索引，string类型。row[1]为Series类型:'date,open'\high\close\low\volume\amoun或price——change等，前面6项和tushare 对应
